class JSON
{
	parse(src, jsonize:=false)
	{
		...
		static esc_seq := Map("`"", "`"", "/", "/", "b", "`b", "f", "`f", "n", "`n", "r", "`r", "t", "`t")
		...
		{
			StrReplace(src, '{', '{', false, c1), StrReplace(src, '}', '}', false, c2)
			if (c1 != c2)
		...
		while ((ch := SubStr(src, ++pos, 1)) != "")
		{
			...
			if InStr("{[", ch)
			{
				val := (proto := this.jbase(ch, jsonize)) ? proto.new() : Map()
				, obj[is_array? obj.Length+1 : key] := val
	...
	jbase(ch, jsonize) {
		jsonize ? Map("{", JSON.object, "[", JSON.array) : Map("{", 0, "[", 0)

		result := ""

		if (ch == "{") {
			result := jsonize ? JSON.object : 0
		} else if (ch == "[") {
			result := jsonize ? JSON.array : 0
		}
