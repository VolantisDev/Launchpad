class EntityEvents {
    static ENTITY_PREPARE := 0x4035
    static ENTITY_PRESAVE := 0x4040
    static ENTITY_CREATED := 0x4045
    static ENTITY_UPDATED := 0x4050
    static ENTITY_PREDELETE := 0x4054
    static ENTITY_DELETED := 0x4055
    static ENTITY_LOADED := 0x4060
    static ENTITY_RESTORED := 0x4062
    static ENTITY_REFRESH := 0x4065
    static ENTITY_CUSTOM_DATA_LAYERS := 0x4070
    static ENTITY_CUSTOM_DATA_LAYERS_ALTER := 0x4071
    static ENTITY_CUSTOM_DATA_POPULATE := 0x4075
    static ENTITY_CUSTOM_DATA_ALTER := 0x4076
    static ENTITY_VALIDATE := 0x4080
    static ENTITY_FIELD_DEFINITIONS := 0x4085
    static ENTITY_FIELD_DEFINITIONS_ALTER := 0x4087
    static ENTITY_FIELD_GROUPS := 0x4090
    static ENTITY_FIELD_GROUPS_ALTER := 0x4092
    static ENTITY_REFERENCE_ENTITY_SAVED := 0x4095
}
